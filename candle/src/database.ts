...
export function getDatabase({ overrideDirectory }: {
...
export function saveNewProcess(command: string, cwd: string, pid: number, port?: number): number {
    ...
    const result = db.run('insert into processes(command, working_directory, pid, start_time, assigned_port) values(?, ?, ?, ?, ?)', [command, cwd, pid, Math.floor(Date.now() / 1000), port || null]);
    return result.lastInsertRowid as number;
}
export function createProcessEntry(commandName: string, commandStr: string, cwd: string, assignedPort: number): number {
    const db = getDatabase();
    const result = db.run('insert into processes(command_name, command_str, working_directory, assigned_port, start_time, pid) values(?, ?, ?, ?, ?, ?)', [commandName, commandStr, cwd, assignedPort, Math.floor(Date.now() / 1000), 0]);
    return result.lastInsertRowid as number;
}
export function updateProcessWithPid(launchId: number, pid: number): void {
    const db = getDatabase();
    db.run('update processes set pid = ? where launch_id = ?', [pid, launchId]);
}
export function updateProcessWithWrapperPid(launchId: number, wrapperPid: number): void {
    const db = getDatabase();
    db.run('update processes set wrapper_pid = ? where launch_id = ?', [wrapperPid, launchId]);
}
export function updateProcessWithPids(launchId: number, pid: number, wrapperPid: number): void {
    const db = getDatabase();
    db.run('update processes set pid = ?, wrapper_pid = ? where launch_id = ?', [pid, wrapperPid, launchId]);
}
export function setProcessExited(launchId: number, exitCode: number): void {
    const db = getDatabase();
    db.run('update processes set end_time = ?, is_running = ?, exit_code = ? where launch_id = ?', [Math.floor(Date.now() / 1000), RunningStatus.stopped, exitCode, launchId]);
}
export function logLine(launchId: number, lineNumber: number, content: string, logType: LogType): void {
    const db = getDatabase();
    db.run('insert into process_output(launch_id, line_number, content, log_type) values(?, ?, ?, ?)', [launchId, lineNumber, content, logType]);
}
...
export function findExistingProcess(commandStr: string, cwd: string): ProcessRow | undefined {
    const db = getDatabase();
    return db.get('select * from processes where command_str = ? and working_directory = ? and is_running = ?', [commandStr, cwd, RunningStatus.running]);
}
